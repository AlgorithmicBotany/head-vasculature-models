...
# A model of vasculature for gerbera

![Gerbera sequence](gerbera_sequence.png)
...
## Phyllotaxis (*.prims) 
The phllotaxis patterning of incipient primordia produces by an accompanying [Gerbera phyllotaxis model](https://github.com/AlgorithmicBotany/head-vasculature-models/tree/main/oofs/ext/Gerbera-Phyllotaxis). The primordia positions on the receptacle are precomputed and stored as a text file (*.prims) input to the vasculature model. It lists the timestamped position (on the developing receptacle) that primordia was patterned.
...
**NOTE** The binary file **gerbera_contours_weights.raw** (240MB) must be downloaded separately at [algorithmicbotany.org](http://algorithmicbotany.org/).
